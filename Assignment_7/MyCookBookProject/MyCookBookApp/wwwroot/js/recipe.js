//Ensures that JavaScript functionality occurs after HTML elements are fully loaded
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll("#editCategoryList .dropdown-item").forEach(item => {
        item.addEventListener("click", function (event) {
            ...
            // Update button text to display selected categories
            document.getElementById("editCategoryDropdown").innerText = selectedEditCategories.length > 0 ? selectedEditCategories.map(id => document.querySelector(`#editCategoryList [data-value="${id}"]`).innerText).join(", ") : "Select Categories";
            // Update hidden input with selected categories
    ...
    document.getElementById("editCategoryDropdown").innerText = selectedCategories.length > 0 ? selectedCategories.map(id => document.querySelector(`#editCategoryList [data-value="${id}"]`).innerText).join(", ") : "Select Categories";
    ...
    window.editRecipe = function (recipeId) {
        ...
        .then(recipe => {
            ...
            Array.isArray(recipe.instructions) ? recipe.instructions.join("\n") : "";
                ...
                let selectedCategoryNames = selectedCategories.map(catId => categoryMapping[catId] || "Unknown");
                ...
                categoryOptions.forEach(option => {
                    let categoryValue = parseInt(option.getAttribute("data-value"), 10); // Convert option value to a number
                    if (selectedCategories.includes(categoryValue)) {
                        option.classList.add("active"); // Visually indicate selected categories
                    } 
                    else {
                ...
                document.getElementById("editCategoryDropdown").innerText = selectedCategoryNames.length > 0 ? selectedCategoryNames.join(", ") : "Select Categories";
