...
namespace MyCookBookApi.Repositories
{
    ...
    public class FirebaseDbRecipeRepository : IRecipeRepository
    {
        ...
        public FirebaseDbRecipeRepository()
        {
            ...
            _firestoreDb = FirestoreDb.Create("mycookbookappdb-2025");
        }
        ...
        private async Task<List<Recipe>> GetAllRecipesAsync()
        {
            ...
            return snapshot.Documents.Select(doc => doc.ConvertTo<Recipe>()).ToList();
        }
        ...
        private async Task<List<Recipe>> SearchRecipesAsync(RecipeSearchRequest searchRequest)
        {
            Query query = _firestoreDb.Collection(CollectionName);
            QuerySnapshot snapshot = await query.GetSnapshotAsync();
            return snapshot.Documents.Select(doc => doc.ConvertTo<Recipe>()).Where(r => string.IsNullOrEmpty(searchRequest.Keyword) || r.Name.Contains(searchRequest.Keyword, StringComparison.OrdinalIgnoreCase) || r.Summary.Contains(searchRequest.Keyword, StringComparison.OrdinalIgnoreCase)).ToList();
        }
